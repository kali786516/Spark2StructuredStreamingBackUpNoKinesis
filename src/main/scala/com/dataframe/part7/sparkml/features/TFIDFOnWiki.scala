package com.dataframe.part7.sparkml.features
...
object TFIDFOnWiki {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val logger = Logger.getLogger("TFIDFOnWiki")
    Logger.getLogger("org").setLevel(Level.WARN)
    Logger.getLogger("akka").setLevel(Level.WARN)

    val conf = new SparkConf().setAppName("Features Extraction TF-IDF").setMaster("local[*]")
    val sc = new SparkContext(conf)
    val wFile = sc.textFile("sparkMLDataSets/wiki_people.csv")
    ...
    // keyset gets all the keyps from map (distinct keys)
