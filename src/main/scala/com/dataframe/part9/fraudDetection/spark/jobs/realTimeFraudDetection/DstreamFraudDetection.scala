package com.dataframe.part9.fraudDetection.spark.jobs.realTimeFraudDetection

import com.dataframe.part9.fraudDetection.cassandra.dao.{CreditcardTransactionRepository, KafkaOffsetRepository}
import com.dataframe.part9.fraudDetection.cassandra.{CassandraConfig, CassandraDriver}
import com.dataframe.part9.fraudDetection.config.Config
import com.dataframe.part9.fraudDetection.creditcard.Schema
import com.dataframe.part9.fraudDetection.kafka.KafkaConfig
import com.dataframe.part9.fraudDetection.spark.{DataReader, GracefulShutdown, SparkConfig}
import com.dataframe.part9.fraudDetection.spark.jobs.SparkJob
  ...
  val connector            = CassandraConnector(sparkSession.sparkContext.getConf)
    ...
    transactionStream.foreachRDD(rdd => {
      ...
      if (!rdd.isEmpty()) {
        ...
        val distanceUdf            = udf(com.dataframe.part9.fraudDetection.utils.Utils.getDistance _)
